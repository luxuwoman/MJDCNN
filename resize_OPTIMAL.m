% resize OPTIMAL dataset depend on image indexs of train, cross validation and test dataset
clc;
clear;
close all;
...
for alpha=1:10
    alpha
imagePath = './data_pre/OPTIMAL/';
load(['./data_split/OPTIMAL/traincvtest_', num2str(alpha, '%d'), '.mat']);
...
for k=1:n_3
    ...
    C_test = str2num(B_test);
    img_test = [img_test; C_test];
end
save(['./data_split/OPTIMAL/trainCVtest_index_', num2str(alpha, '%d'),'.mat'],'img_train','img_CV','img_test');
...
for q=3:numFiles
    ...
    imagePath_1 = strcat('./data_pre/OPTIMAL_CLASS_',sizedir,'/', num2str(alpha, '%d'),...
    '/OPTIMAL_',sizedir,'_train/',subfoldername);
    mkdir(imagePath_1);
    ...
    imagePath_2 = strcat('./data_pre/OPTIMAL_CLASS_',sizedir,...
        '/', num2str(alpha, '%d'),'/OPTIMAL_',sizedir,'_CV/',subfoldername);
    mkdir(imagePath_2);
    c = (q-3) * n_cv + 1;
    d = (q-2) * n_cv;
    for p_CV=c:d
        file_cv = img_CV(p_CV,1);
        filenames_CV = strcat(imagePath,'/',subfoldername,'/',filename_tif(2).name);
        res_CV = imresize(imread(filenames_CV), data_resize);
        imwrite(res_CV, strcat(imagePath_2,'/',filename_tif(file_cv).name));
    end
    
    imagePath_3 = strcat('./data_pre/OPTIMAL_CLASS_',sizedir,...
        '/', num2str(alpha, '%d'),'/OPTIMAL_',sizedir,'_test/',subfoldername);
    mkdir(imagePath_3);
    for p_test=a:b
