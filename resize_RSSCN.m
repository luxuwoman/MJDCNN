% resize RSSCN dataset depend on image indexs of train, cross validation and test dataset
clc;
clear;
close all;
...
for alpha=1:10
    alpha
imagePath = './data_pre/RSSCN/';
load(['./data_split/RSSCN/traincvtest_', num2str(alpha, '%d'), '.mat']);
...
save(['./data_split/RSSCN/trainCVtest_index_', num2str(alpha, '%d'),'.mat'],'img_train','img_CV','img_test');
...
for q=3:numFiles
    ...
    imagePath_1 = strcat('./data_pre/RSSCN_CLASS_',sizedir,'/', num2str(alpha, '%d'),...
    '/RSSCN_',sizedir,'_train/',subfoldername);
    mkdir(imagePath_1);
    ...
    imagePath_2 = strcat('./data_pre/RSSCN_CLASS_',sizedir,...
        '/', num2str(alpha, '%d'),'/RSSCN_',sizedir,'_CV/',subfoldername);
    mkdir(imagePath_2);
    c = (q-3) * n_cv + 1;
    d = (q-2) * n_cv;
    for p_CV=c:d
    ...
    imagePath_3 = strcat('./data_pre/RSSCN_CLASS_',sizedir,...
        '/', num2str(alpha, '%d'),'/RSSCN_',sizedir,'_test/',subfoldername);
    mkdir(imagePath_3);
    for p_test=a:b
